...
import type { CourseProgress, ExerciseAttemptResult, LearningEntry, StudiedClip } from "./types";
...
export async function startStudyingClip(movieId: string, clipId: string): Promise<StudiedClip> { const response = await read<{ clip: StudiedClip }>(await authFetch(`${API}/movies/${movieId}/clips/${clipId}/start`, { method: "POST" })); return response.clip; }
export async function listStudiedClips(): Promise<StudiedClip[]> { const response = await read<{ items: StudiedClip[] }>(await authFetch(`${API}/clips`)); return response.items; }
export async function getStudiedClip(clipId: string): Promise<StudiedClip> { const response = await read<{ clip: StudiedClip }>(await authFetch(`${API}/clips/${clipId}`)); return response.clip; }
