    ...
    <div className="flex h-[calc(100dvh-65px)] overflow-hidden bg-slate-100">
      <aside className="hidden w-72 shrink-0 flex-col border-r border-slate-200 bg-white lg:flex">
        <div className="border-b border-slate-100 p-6 text-center">
          <CharacterAvatar size="lg" name={chat.character.name} avatarUrl={chat.character.avatar_url} />
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 sm:px-6">
          <div className="flex items-center gap-3">
            <CharacterAvatar size="sm" name={chat.character.name} avatarUrl={chat.character.avatar_url} />
            <div>
        ...
        <div ref={scroller} className="min-h-0 flex-1 overflow-y-auto">
          <CharacterMessages messages={chat.messages} greeting={chat.character.greeting} sending={chat.sending} error={chat.error} characterName={chat.character.name} avatarUrl={chat.character.avatar_url} />
        </div>
