  ...
  return (
    <>
      <form onSubmit={submit} className="mt-8 grid gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <Field label="Вертикальный постер"><Input name="thumbnail" type="file" accept="image/jpeg,image/png,image/webp,image/avif" required disabled={uploading} /></Field>
        ...
        {movies.map((movie) => (
          <div key={movie.id} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 items-center gap-4">
              <div className="aspect-[2/3] w-14 shrink-0 overflow-hidden rounded-lg bg-slate-100">
                {movie.thumbnail_url ? (
                  ...
                  <div className="flex h-full items-center justify-center px-1 text-center text-[10px] text-slate-400">Нет постера</div>
                )}
              </div>
